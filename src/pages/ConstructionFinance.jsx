...
const ConstructionFinanceApp = () => {
  const [categories, setCategories] = useState(() => { const s = localStorage.getItem('cfCategories'); return s ? JSON.parse(s) : DEFAULT_CATEGORIES; });
  ...
  const calcMetrics = (site) => {
    ...
    const boqTotal = site.contractors.reduce((s, c) => s + (c.boqTotal || 0), 0);
    const conBalance = boqTotal - conPaid;
    const retentionTotal = site.contractors.reduce((s, c) => s + (c.boqTotal || 0) * (c.retention || 0) / 100, 0);
  ...
  const addSite = () => {
    ...
    setSites([...sites, s]); setCurrentSiteId(s.id); setNewSite({ name: '', location: '', budget: '', startDate: '' });
  };
  const deleteSite = (id) => {
    const rem = sites.filter(s => s.id !== id); setSites(rem);
  ...
  const addContractor = () => {
    ...
    setNewContractor(emptyCon); setCustomContractorName(''); setContractorNameMode('preset');
  };
  ...
  const addCategory = () => { const t = newCategoryInput.trim(); if (t && !categories.includes(t)) setCategories([...categories, t]); setNewCategoryInput(''); setShowAddCategory(false); };
  const removeCategory = (cat) => setCategories(categories.filter(c => c !== cat));

  // ── AI ──
  const getContext = () => {
    const site = getCurrentSite(); const m = calcMetrics(site);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 600, system: `You are an expert construction financial advisor.\n\nData:\n${getContext()}`, messages: messages.filter(m => m.role !== 'system').concat([{ role: 'user', content: msg }]) }) });
      const data = await res.json();
      setMessages(p => [...p, { role: 'assistant', content: data.content[0]?.text || 'Error.' }]);
    } catch (e) { setMessages(p => [...p, { role: 'assistant', content: `Error: ${e.message}` }]); } finally { setLoading(false); }
  };
  ...
  const exportPDF = () => {
    ...
    const boqChartData = site.contractors.map(c => ({ name: c.name.split(' ')[0], boq: c.boqTotal || 0, paid: c.paid || 0, balance: (c.boqTotal || 0) - (c.paid || 0) }));
    const matChartData = site.materials.map(mat => ({ name: mat.name, cost: mat.quantity * mat.unitCost }));
    const totalBOQ = site.contractors.reduce((s, c) => s + (c.boqTotal || 0), 0);
    const totalPaid = site.contractors.reduce((s, c) => s + (c.paid || 0), 0);
    const totalRet = site.contractors.reduce((s, c) => s + (c.boqTotal || 0) * (c.retention || 0) / 100, 0);
    ...
    const contractorRows = site.contractors.map((c, i) => {
      ...
      return `<tr style="background:${i%2===0?'white':'#f9fafb'}">
        ...
        <td><div style="background:#e5e7eb;border-radius:4px;height:8px;width:80px"><div style="background:${parseFloat(pctPaid)>80?'#dc2626':'#059669'};height:8px;border-radius:4px;width:${Math.min(pctPaid,100)}%"></div></div><span style="font-size:10px">${pctPaid}%</span></td>
    ...
    // SVG bar charts
    const maxBOQ = Math.max(...boqChartData.map(d => d.boq), 1);
    const chartW = 500, chartH = 160, barW = Math.min(40, (chartW - 60) / (boqChartData.length * 3 + 1));
    const boqBars = boqChartData.map((d, i) => {
      const x = 40 + i * (chartW - 60) / boqChartData.length;
      const boqH = (d.boq / maxBOQ) * (chartH - 30);
      const paidH = (d.paid / maxBOQ) * (chartH - 30);
      return `<g><rect x="${x}" y="${chartH-30-boqH}" width="${barW}" height="${boqH}" fill="#7c3aed" opacity="0.8"/>
        <rect x="${x+barW+2}" y="${chartH-30-paidH}" width="${barW}" height="${paidH}" fill="#059669" opacity="0.8"/>
        <text x="${x+barW}" y="${chartH-14}" text-anchor="middle" font-size="9" fill="#374151">${d.name.substring(0,8)}</text></g>`;
    }).join('');
    const boqSVG = `<svg viewBox="0 0 ${chartW} ${chartH}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:${chartH}px">
      <line x1="40" y1="10" x2="40" y2="${chartH-30}" stroke="#e5e7eb"/><line x1="40" y1="${chartH-30}" x2="${chartW-10}" y2="${chartH-30}" stroke="#e5e7eb"/>
      ${boqBars}
      <rect x="10" y="5" width="10" height="8" fill="#7c3aed"/><text x="23" y="13" font-size="9" fill="#374151">BOQ</text>
      <rect x="60" y="5" width="10" height="8" fill="#059669"/><text x="73" y="13" font-size="9" fill="#374151">Paid</text>
    </svg>`;

    const matTotal = site.materials.reduce((s,m)=>s+m.quantity*m.unitCost,0);
    const pieAngles = site.materials.map(mat => (mat.quantity*mat.unitCost/Math.max(matTotal,1))*360);
    let cumAngle = -90;
    const piePath = site.materials.map((mat,i) => {
      const angle = pieAngles[i]; const r=60;
      const x1=80+r*Math.cos(cumAngle*Math.PI/180), y1=80+r*Math.sin(cumAngle*Math.PI/180);
      cumAngle+=angle;
      const x2=80+r*Math.cos(cumAngle*Math.PI/180), y2=80+r*Math.sin(cumAngle*Math.PI/180);
      const la=angle>180?1:0;
      return `<path d="M80,80 L${x1},${y1} A${r},${r} 0 ${la},1 ${x2},${y2} Z" fill="${COLORS[i%COLORS.length]}" opacity="0.85"/>`;
    }).join('');
    const pieLegend = site.materials.map((mat,i) => `<text x="160" y="${20+i*16}" font-size="9" fill="#374151">■ ${mat.name}: $${(mat.quantity*mat.unitCost).toLocaleString()}</text>`).join('');
    const pieSVG = `<svg viewBox="0 0 350 160" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:160px">${piePath}${pieLegend}</svg>`;

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Finance Report — ${site.name}</title>
    <style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#1f2937;background:white}.header{background:linear-gradient(135deg,#d97706,#7c3aed);color:white;padding:22px 28px}.header h1{font-size:20px;margin-bottom:4px}.header p{font-size:12px;opacity:.85}.content{padding:22px 28px}.kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px}.kpi{background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:10px 12px}.kpi-label{font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px}.kpi-value{font-size:18px;font-weight:800}.section-title{font-size:13px;font-weight:700;color:#1f2937;margin:20px 0 10px;padding-bottom:6px;border-bottom:2px solid #d97706}.chart-row{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px}.chart-box{background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:12px}.chart-box h4{font-size:11px;font-weight:700;color:#374151;margin-bottom:8px}table{width:100%;border-collapse:collapse;margin-bottom:8px}thead th{background:#1f2937;color:white;padding:6px 8px;font-size:9px;text-align:left;white-space:nowrap}tbody td{padding:5px 8px;font-size:10px;border-bottom:1px solid #f0f0f0;vertical-align:middle}tfoot td{padding:6px 8px;font-size:10px;font-weight:800;background:#fef3c7;color:#92400e}.progress{background:#e5e7eb;border-radius:4px;height:6px}.footer{margin-top:20px;font-size:9px;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:8px;display:flex;justify-content:space-between}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head>
    <body>
    <div class="header"><h1>🏗️ Construction Finance Report</h1><p>${site.name} | ${site.location||''} | Generated: ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}</p></div>
    <div class="content">
    <div class="kpi-grid">
      ...
      <div class="kpi"><div class="kpi-label">Total Contractor Paid</div><div class="kpi-value" style="color:#059669">$${m.conPaid.toLocaleString()}</div></div>
      <div class="kpi"><div class="kpi-label">Contractor Balance</div><div class="kpi-value" style="color:#d97706">$${m.conBalance.toLocaleString()}</div></div>
      <div class="kpi"><div class="kpi-label">Retention Held</div><div class="kpi-value" style="color:#0369a1">$${m.retentionTotal.toLocaleString()}</div></div>
      <div class="kpi"><div class="kpi-label">Material Spend</div><div class="kpi-value" style="color:#7c3aed">$${m.matSpent.toLocaleString()}</div></div>
    </div>
    <div class="section-title">📊 Visual Summary</div>
    <div class="chart-row">
      <div class="chart-box"><h4>Contractor BOQ vs Paid</h4>${boqSVG}</div>
      <div class="chart-box"><h4>Material Cost Breakdown</h4>${pieSVG}</div>
    </div>
    ...
    <tfoot><tr><td colspan="6">TOTAL (${site.contractors.length})</td><td style="color:#7c3aed">$${totalBOQ.toLocaleString()}</td><td style="color:#059669">$${totalPaid.toLocaleString()}</td><td></td><td style="color:#0369a1">$${totalRet.toLocaleString()}</td><td style="color:#d97706">$${(totalBOQ-totalPaid).toLocaleString()}</td><td colspan="4"></td></tr></tfoot></table>
    ...
    <tbody>${matRows}</tbody>
    <tfoot><tr><td colspan="7">TOTAL (${site.materials.length} items)</td><td style="color:#7c3aed">$${m.matSpent.toLocaleString()}</td><td colspan="4"></td></tr></tfoot></table>
    <div class="footer"><span>🏗️ Construction Finance Manager | Confidential</span><span>Status: ${site.status||'In Progress'} | Start: ${site.startDate||'—'}</span></div>
    </div>
  ...
  const exportCSV = () => {
    ...
    csv += `\nCONTRACTORS BOQ\nName,Scope,Unit,Price/Unit,Quantity,BOQ Total,Paid,Retention%,Balance\n`;
    site.contractors.forEach(x => { csv += `${x.name},${x.scope},${x.unit},${x.pricePerUnit},${x.quantity},${x.boqTotal},${x.paid},${x.retention}%,${x.boqTotal-x.paid}\n`; });
    csv += `\nPAYMENT HISTORY\nContractor,Date,Amount,Reference,Method,Description\n`;
    site.contractors.forEach(c => { (c.payments||[]).forEach(p => { csv += `${c.name},${p.date},$${p.amount},${p.reference||''},${p.method||''},${p.description||''}\n`; }); });
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `finance_${site.name}_${Date.now()}.csv`; a.click();
  };
  ...
  const statusBadge = (s) => { const map = { Completed: ['#d1fae5','#065f46'], 'In Progress': ['#fef3c7','#92400e'], Pending: ['#f3f4f6','#374151'] }; const [bg, col] = map[s] || map.Pending; return <span style={{ background: bg, color: col, padding: '3px 9px', borderRadius: '12px', fontSize: '11px', fontWeight: '700' }}>{s}</span>; };
  ...
  // Chart data
  const contractorChartData = site?.contractors.map(c => ({ name: c.name.length > 12 ? c.name.substring(0,12)+'…' : c.name, 'BOQ Total': c.boqTotal||0, 'Paid': c.paid||0, 'Balance': (c.boqTotal||0)-(c.paid||0) })) || [];
  const materialChartData = site?.materials.map(m => ({ name: m.name, value: m.quantity * m.unitCost, qty: m.quantity, unit: m.unit })) || [];
  const globalChartData = sites.map(s => { const m = calcMetrics(s); return { name: s.name.length>10?s.name.substring(0,10)+'…':s.name, Budget: s.budget, Spent: m.spent, 'BOQ': m.boqTotal, 'Paid': m.conPaid }; });
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 82px)' }}>
        ...
        <div style={{ width: '230px', minWidth: '230px', background: 'white', borderRight: '1px solid #e5e7eb', padding: '16px', overflowY: 'auto' }}>
            ...
            return (
              <div key={s.id} style={{ position: 'relative', marginBottom: '8px' }}>
                <div onClick={() => setCurrentSiteId(s.id)} style={{ padding: '10px 32px 10px 10px', background: currentSiteId===s.id?'#fef3c7':'#f9fafb', border: `2px solid ${currentSiteId===s.id?'#d97706':'transparent'}`, borderRadius: '8px', cursor: 'pointer' }}>
                  ...
                  <div style={{ fontSize: '10px', color: '#d97706', fontWeight: '600', marginTop: 2 }}>${(s.budget/1000).toFixed(0)}k · {m.pct}% used</div>
          ...
          {/* Payments quick link per contractor */}
          {site && (
            <div style={{ marginTop: '12px' }}>
              ...
              {site.contractors.map(c => {
                ...
                return (
                  <div key={c.id} onClick={() => { setActiveTab('payments'); setSelectedContractorId(c.id); }} style={{ padding: '8px 10px', background: selectedContractorId===c.id&&activeTab==='payments'?'#ede9fe':'#f9fafb', borderRadius: '6px', cursor: 'pointer', marginBottom: '6px', border: `1px solid ${selectedContractorId===c.id&&activeTab==='payments'?'#7c3aed':'transparent'}` }}>
              ...
              {site.contractors.length === 0 && <div style={{ fontSize: '11px', color: '#9ca3af', padding: '8px' }}>No contractors yet</div>}
            </div>
          )}
        </div>
                ...
                <div style={{ background: 'white', padding: '18px', borderRadius: '10px', marginBottom: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: '10px' }}>
                    <div><div style={{ fontSize:'11px',color:'#6b7280',marginBottom:'4px' }}>Scope *</div>
                      <select value={newContractor.scope} onChange={e=>setNewContractor({...newContractor,scope:e.target.value})} style={inp}><option value="">Select</option>{CONTRACTOR_SCOPES.map(s=><option key={s}>{s}</option>)}<option value="__custom__">Other</option></select>
                      {newContractor.scope==='__custom__' && <input placeholder="Custom scope" style={{...inp,marginTop:'6px'}} onChange={e=>setNewContractor({...newContractor,scope:e.target.value==='__custom__'?'':e.target.value})}/>}
                    </div>
                  ...
                  {boqPreview > 0 && <div style={{ marginTop:'10px', padding:'10px 14px', background:'#fef3c7', borderRadius:'6px', fontSize:'13px', color:'#92400e', fontWeight:'700', display:'flex', gap:'20px' }}><span>BOQ: ${boqPreview.toLocaleString()}</span></div>}
                      ...
                      {site.contractors.map((c,i) => {
                        ...
                        return (
                          <tr key={c.id} style={{ background: i%2===0?'white':'#fafafa' }}>
                            ...
                            <td style={TD}>
                              <button onClick={()=>{setSelectedContractorId(c.id);setActiveTab('payments');}} style={{...btn('#ede9fe','#7c3aed'),fontSize:'11px',padding:'5px 10px'}}><CreditCard size={12}/> {(c.payments||[]).length}</button>
                            </td>
