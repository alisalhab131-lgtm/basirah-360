...
export default function DashboardPage({ materials, contractors, loans, returns, getLoanRemainingQty }) {
  ...
  const [drillSite, setDrillSite] = useState(null);
  ...
  const totalGood = returns.filter(r => r.returned_condition === 'Good').length;
  const totalWorn = returns.filter(r => r.returned_condition === 'Worn').length;
  const totalDamaged = returns.filter(r => r.returned_condition === 'Damaged').length;
  ...
  const conditionData = [
    ...
    { name: 'Worn', value: totalWorn },
    { name: 'Damaged', value: totalDamaged },
  ].filter(d => d.value > 0);

  const generateRows = () => {
    let rows = [];
    if (['ALL', 'AVAILABLE'].includes(cardFilter) && !drillSite) {
      materials.forEach(m => {
    ...
    if (['ALL', 'LENDED', 'OVERDUE'].includes(cardFilter)) {
      loans.forEach(l => {
        ...
        if (drillSite && l.site_name?.trim() !== drillSite) return;
        rows.push({
  ...
  const clearFilters = () => { setCardFilter('ALL'); setDrillSite(null); };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
        <KpiCard icon={LayoutGrid} label="Total Stock" value={totalStock} color={THEME.accentBlue} isActive={cardFilter === 'ALL'} onClick={clearFilters} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '28px' }}>
        ...
        <KpiCard icon={CheckCircle} label="Good Returns" value={totalGood} color={THEME.accentEmerald} subtitle="Returned in good condition" />
        <KpiCard icon={Wrench} label="Worn Returns" value={totalWorn} color={THEME.accentAmber} subtitle="Needs service review" />
        <KpiCard icon={XCircle} label="Damaged Returns" value={totalDamaged} color={THEME.accentCrimson} subtitle="Requires repair or scrap" />
        ...
        <KpiCard icon={CheckCircle} label="Return Health" value={`${healthRate}%`} color={THEME.accentEmerald} subtitle="Good of all returns" />
        <KpiCard icon={Package} label="Total Returned" value={totalReturned} color={THEME.accentBlue} subtitle="All-time recoveries" />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
        <div style={STYLES.box}>
          <div style={STYLES.label}>Deployments by Site — click to filter</div>
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <BarChart data={siteChartData} onClick={(d) => d?.activeLabel && setDrillSite(d.activeLabel)}>
      ...
      <div style={STYLES.box}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={STYLES.label}>
            Asset Tracking {cardFilter !== 'ALL' && `· ${cardFilter}`}
            {drillSite && ` · Site: ${drillSite}`}
          </div>
          {(cardFilter !== 'ALL' || drillSite) && (
            <button onClick={clearFilters} style={{ background: 'none', border: 'none', color: THEME.accentBlue, cursor: 'pointer', fontSize: '12px', fontWeight: '600' }}>
              Clear Filters
            </button>
          )}
        </div>
