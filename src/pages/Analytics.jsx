...
export default function AnalyticsPage({ materials, contractors, loans, returns, getLoanRemainingQty }) {
  ...
  const siteStats = Object.values(
    loans.reduce((acc, l) => {
      ...
      if (!acc[site]) acc[site] = { name: site, loaned: 0, returned: 0, overdue: 0, damaged: 0, worn: 0, good: 0, loanIds: [] };
  ...
  const contractorStats = contractors.map(c => {
    ...
    return { ...c, loaned, returned, good, worn, damaged, overdue,
  ...
  const siteLoanHistory = selectedSite ? loans.filter(l => (l.site_name || '').trim() === selectedSite.name).map(l => {
    const lReturns = returns.filter(r => Number(r.loan_id) === l.id);
    return { ...l, retQty: lReturns.reduce((s, r) => s + Number(r.returned_quantity || r.quantity || 0), 0), remaining: getLoanRemainingQty(l.id), lReturns };
  }) : [];

  const selectedContractor = drillType === 'contractor' ? contractorStats.find(c => String(c.id) === String(drillValue)) : null;
  const contractorLoanHistory = selectedContractor ? loans.filter(l => String(l.contractor_id) === String(selectedContractor.id)).map(l => {
    const lReturns = returns.filter(r => Number(r.loan_id) === l.id);
    return { ...l, retQty: lReturns.reduce((s, r) => s + Number(r.returned_quantity || r.quantity || 0), 0), remaining: getLoanRemainingQty(l.id), lReturns };
  }) : [];
  ...
  if (drillType === 'site' && selectedSite) return (
    <div>
      ...
      <p style={{ color: THEME.textMuted, fontSize: '13px', marginBottom: '24px' }}>Full utilization history and condition breakdown</p>
      <StatGrid stats={[
                ...
                : siteLoanHistory.map(l => {
                  ...
                  return (
                    <tr key={l.id}>
                      ...
                      <td style={STYLES.td}>{l.quantity}</td><td style={STYLES.td}>{l.retQty}</td>
                      <td style={STYLES.td}>{l.remaining}</td>
                      <td style={STYLES.td}>{[...new Set(l.lReturns.map(r => r.returned_condition))].join(', ') || '—'}</td>
            ...
            <tbody>
              {contractorLoanHistory.length === 0
                ...
                : contractorLoanHistory.map(l => {
                  const isOverdue = l.expected_return_date && new Date(l.expected_return_date) < new Date() && l.remaining > 0;
                  const condMap = {};
                  l.lReturns.forEach(r => { condMap[r.returned_condition] = (condMap[r.returned_condition] || 0) + Number(r.returned_quantity || r.quantity || 0); });
                  return (
                    <tr key={l.id}>
                      <td style={STYLES.td}>{l.material_name}</td><td style={STYLES.td}>{l.site_name || '—'}</td>
                      <td style={STYLES.td}>{l.quantity}</td><td style={STYLES.td}>{l.retQty}</td>
                      <td style={STYLES.td}>
                        {Object.entries(condMap).length > 0
                          ? Object.entries(condMap).map(([cond, qty]) => <span key={cond} style={{ marginRight: '6px' }}>{BADGE(CONDITION_COLORS[cond] || THEME.textMuted, `${cond}: ${qty}`)}</span>)
  ...
  return (
    <div>
      ...
      <div style={STYLES.box}>
        ...
        <table style={STYLES.table}>
          <thead><tr>
            <th style={STYLES.th}>Site</th><th style={STYLES.th}>Loaned</th><th style={STYLES.th}>Returned</th>
          ...
          <tbody>
            {siteStats.length === 0
              ? <tr><td colSpan={9} style={{ ...STYLES.td, textAlign: 'center', color: THEME.textMuted }}>No site data</td></tr>
              : siteStats.map(s => (
                <tr key={s.name} style={{ cursor: 'pointer' }} onClick={() => { setDrillType('site'); setDrillValue(s.name); }}>
                  ...
                  <td style={STYLES.td}>{s.loaned}</td><td style={STYLES.td}>{s.returned}</td>
        ...
        <table style={STYLES.table}>
          ...
          <tbody>
            {contractorStats.length === 0
              ? <tr><td colSpan={10} style={{ ...STYLES.td, textAlign: 'center', color: THEME.textMuted }}>No contractor data</td></tr>
              : contractorStats.map(c => (
                <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => { setDrillType('contractor'); setDrillValue(c.id); }}>
                  ...
                  <td style={STYLES.td}>{c.loaned}</td><td style={STYLES.td}>{c.returned}</td>
